...
|------|------|
| `你好，世界！测试中文不乱码` | ✅ 正常 |
| `????` / `浣犲ソ` / `乱码` | ❌ 插件未生效，检查部署和重启 |

也可测试中文文件读写：

```powershell
echo "中文内容测试" > test-编码.txt
Get-Content test-编码.txt
Remove-Item test-编码.txt
```

### ② export → $env: 自动转换

让 Agent 执行以下 Linux export 语法的命令（**不要手动加编码前缀**，让 Agent 直接输出 `export`）：
...
| 结果 | 判定 |
|------|------|
| `HelloWorld` | ✅ 正常 — export 被自动转换为 `$env:MY_TEST_VAR="HelloWorld"` |
| 报错 `'export' 不是内部或外部命令` | ❌ 插件未生效或未重启 |

> ⚠️ 注意：测试时命令中不应包含 `$OutputEncoding=` 等前缀，插件会自动处理。如果手动添加编码前缀，`export` 不在命令开头，插件不会触发转换。

多变量 export 测试：

```bash
export DB_HOST=localhost DB_PORT=5432
echo "$env:DB_HOST:$env:DB_PORT"
```
...
|------|------|
| 正常显示仓库状态，无交互弹窗 | ✅ 正常 |
| 命令前出现 `$env:CI="true";$env:DEBIAN_FRONTEND=...` 长串 | ⚠️ 有非 ShellFix 注入，检查 non-interactive-env 禁用 |
| 卡住等待编辑器/凭据/分页器 | ❌ Git 未免交互，检查 ShellFix shell.env 是否生效 |
...
| Git 免交互 | `git status` | 正常执行，命令干净无多余前缀 |
