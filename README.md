...
用户操作（Ctrl+P 或输入框）
  ...
  │   │   ├─ 返回 { output } → appendPrompt（填入输入框）
  │   │   ├─ 返回 { toast }  → showToast（通知）
  │   │   └─ 返回 null       → 未处理
  │   │
  │   └─ 未处理 → api.client.tui.executeCommand()
...
```
~/.config/opencode/
├── plugins/shell-fix.ts           ← 服务器插件
├── plugins/shell-fix-tui.ts       ← TUI 插件
...
└── docs/
    ├── D0-ShellFix-全功能设计.md  # 完整设计文档
    ├── 01-方案评估.md             # 原始方案技术审查
    ├── 02-完整修复方案.md          # 修正后设计方案
    ├── 03-安装与使用.md           # 安装指南（v1.0）
    ├── 09-API-探索报告.md         # 平台 API 探索结论
    ├── Audit-ShellFix-V1.9-审核报告.md
    └── _Draft/                   # 历史开发方案草案
```
